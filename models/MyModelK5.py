...
class ActorNetwork(nn.Module):
    ...
    def forward(self, x, user_type_onehot):
        ...
        user_tower_list = torch.concat([tower1_out,tower2_out,tower3_out,tower4_out,tower5_out], dim =2) #[bs,dim, 6]
...
class CriticNetwork(nn.Module):
    ...
    def forward(self, base_tower, labels, user_type_onehot):
        ...
        user_flag = user_type_onehot.unsqueeze(1)
        a_user_tower = torch.concat([tower1_out,tower2_out,tower3_out,tower4_out,tower5_out], dim =2)     #[bs,dim, 6]
...
class LiveRecoModelK5(nn.Module):
    def __init__(self, 
                 input_network = None,
                 base_output_dim = 128
                 ):
        super(LiveRecoModelK5, self).__init__()
    ...
    def forward(self, input):
        ...
        user_type = [1 if x in [1, 2] else x-1 for x in user_type]
        user_type_indices = torch.tensor(user_type, dtype=torch.long).cuda(device_controller.get_device()) - 1       # [1, 6]--->范围是0~5
        user_type_onehot = F.one_hot(user_type_indices, num_classes=5)  # [batch_size, user_type_num]
    ...
    def get_probs(self, input):
        ...
        user_type_indices = torch.tensor(user_type, dtype=torch.long).cuda(
            device_controller.get_device()) - 1  # [1, 6]--->范围是0~5
        user_type_onehot = F.one_hot(user_type_indices, num_classes=5)  # [batch_size, user_type_num]
        cur_action_logits = self.actor_network(cur_common_input_res.detach(), user_type_onehot)
        cur_action_probs = F.softmax(cur_action_logits, dim=1)
