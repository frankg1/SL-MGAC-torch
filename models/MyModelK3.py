...
class ActorNetwork(nn.Module):
    ...
    def forward(self, x, user_type_onehot):
        ...
        user_tower_list = torch.concat([tower1_out,tower2_out,tower3_out], dim =2) #[bs,dim, 6]
...
class CriticNetwork(nn.Module):
    ...
    def forward(self, base_tower, labels, user_type_onehot):
        ...
        user_flag = user_type_onehot.unsqueeze(1)
        a_user_tower = torch.concat([tower1_out,tower2_out,tower3_out], dim =2)     #[bs,dim, 6]
...
class LiveRecoModelK3(nn.Module):
    def __init__(self, 
                 input_network = None,
                 base_output_dim = 128
                 ):
        super(LiveRecoModelK3, self).__init__()
    ...
    def forward(self, input):
        ...
        user_type = [1 if x in [1, 2] else 2 if x in [3, 4] else 3 if x in [5, 6] else x for x in user_type]
        user_type_indices = torch.tensor(user_type, dtype=torch.long).cuda(device_controller.get_device()) - 1       # 原本的范围是1～6，-1之后是0～5。
        user_type_onehot = F.one_hot(user_type_indices, num_classes=3)
