...
class LiveRecoModelSQ(nn.Module):
    # 计算Q值的时候不再softmax
    def __init__(self, 
                 input_network = None,
                 base_output_dim = 128
                 ):
        super(LiveRecoModelSQ, self).__init__()
    ...
    def forward(self, input):
        ...
        tot_q_action_value = torch.sum(tot_q_value * cur_action, dim =1,  keepdim=True)
        custom_loss = -torch.sum(cur_log_action_prob * tot_q_action_value)
        
        actor_loss = custom_loss
        ...
        # ce_loss = torch.sum(F.cross_entropy(cur_action_logits, tot_q_value, reduction='none').view(-1, 1))
        # actor_loss = ce_loss
