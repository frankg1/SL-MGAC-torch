...
class TimeCriticNetwork(nn.Module):
    ...
    def forward(self, cur_x, nxt_x, cur_labels, nxt_labels, not_final, nxt_action_logits, user_type_onehot):
        # define Hyper params
        alpha_time = 1.0
        delta_time = 0.1
        alpha_photo = 2.0
        ...
        nxt_q_value = torch.min(nxt_q1_value, nxt_q2_value).detach()
        nxt_action_probs = F.softmax(nxt_action_logits, dim=1)
        ...
        q_label = cur_time_delta_reward + 0.9 * torch.Tensor(not_final).view(-1, 1).cuda(device_controller.get_device()) * torch.sum(nxt_action_probs * nxt_q_value, dim = 1, keepdim =True)
...
class LiveRecoModelNotQ(nn.Module):
    # 使用原始定义；正常的critic_next_q，用actor网络计算next prob；
    def __init__(self, 
                 input_network = None,
                 base_output_dim = 128
                 ):
        super(LiveRecoModelNotQ, self).__init__()
    ...
    def forward(self, input):
        ...
        nxt_action_logits = self.actor_network(nxt_common_input_res.detach(), user_type_onehot)

        #交替训练 ----> 改成复制。
        time_q_value1, live_time_sup_loss1, photo_time_sup_loss1, time_delta_sup_loss1, sup_loss1, time_rl_loss1 = self.time_critic1(cur_common_input_res, nxt_common_input_res, cur_labels, nxt_labels, not_final, nxt_action_logits, user_type_onehot)
        
        time_q_value2, live_time_sup_loss2, photo_time_sup_loss2, time_delta_sup_loss2, sup_loss2, time_rl_loss2 = self.time_critic2(cur_common_input_res, nxt_common_input_res, cur_labels, nxt_labels, not_final, nxt_action_logits, user_type_onehot)
