...
class SupervisedVisionNetwork(nn.Module):
    ...
    def forward(self, x, user_type_onehot, idx_vec, reward_start, reward_range):
        tower1_out = self.tower1(x)
        user_tower = torch.sigmoid(tower1_out)  # 4 8
...
class TimeCriticNetwork(nn.Module):
    ...
    def forward(self, cur_x, nxt_x, cur_labels, nxt_labels, not_final, user_type_onehot):
        ...
        return cur_q_value, live_time_sup_loss, photo_time_sup_loss, time_delta_sup_loss, sup_loss, rl_loss, cur_q1_action_value, cur_q2_action_value, q_label

class LiveRecoModelK1(nn.Module):
    def __init__(self, 
                 input_network = None,
                 base_output_dim = 128
                 ):
        super(LiveRecoModelK1, self).__init__()
    ...
    def forward(self, input):
        ...
        user_type = cur_features['user_type'] # 是一个list，长度是batch_size
        user_type = [1 if x in [1, 2] else 2  for x in user_type]
        user_type_indices = torch.tensor(user_type, dtype=torch.long).cuda(device_controller.get_device()) - 1       # 原本的范围是1～6，-1之后是0～5。
        user_type_onehot = F.one_hot(user_type_indices, num_classes=2)
        # 等于1 ，性质变了，这个onehot就没用了
        ...
        #交替训练 ----> 改成复制。
        time_q_value1, live_time_sup_loss1, photo_time_sup_loss1, time_delta_sup_loss1, sup_loss1, time_rl_loss1, cur_q11_action_value, cur_q12_action_value, q_label1 = self.time_critic1(cur_common_input_res, nxt_common_input_res, cur_labels, nxt_labels, not_final, user_type_onehot)
        
        time_q_value2, live_time_sup_loss2, photo_time_sup_loss2, time_delta_sup_loss2, sup_loss2, time_rl_loss2, cur_q21_action_value, cur_q22_action_value, q_label2 = self.time_critic2(cur_common_input_res, nxt_common_input_res, cur_labels, nxt_labels, not_final, user_type_onehot)
        ...
        cur_q1_action_value = (1.0 - step_mod) * cur_q11_action_value + step_mod * cur_q21_action_value
        cur_q2_action_value = (1.0 - step_mod) * cur_q12_action_value + step_mod * cur_q22_action_value
        q_label =  (1.0 - step_mod) * q_label1 + step_mod * q_label2
        return cur_action, cur_time_reward, cur_action_probs, actor_loss, time_rl_loss, critic_loss, sup_loss, cur_q1_action_value, cur_q2_action_value, q_label
