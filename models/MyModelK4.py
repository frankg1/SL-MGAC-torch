...
class ActorNetwork(nn.Module):
    ...
    def forward(self, x, user_type_onehot):
        ...
        user_tower_list = torch.concat([tower1_out,tower2_out,tower3_out,tower4_out], dim =2) #[bs,dim, 6]
...
class CriticNetwork(nn.Module):
    ...
    def forward(self, base_tower, labels, user_type_onehot):
        ...
        user_flag = user_type_onehot.unsqueeze(1)
        a_user_tower = torch.concat([tower1_out,tower2_out,tower3_out,tower4_out], dim =2)     #[bs,dim, 6]
...
class LiveRecoModelK4(nn.Module):
    def __init__(self, 
                 input_network = None,
                 base_output_dim = 128
                 ):
        super(LiveRecoModelK4, self).__init__()
    ...
    def forward(self, input):
        ...
        user_type = [3 if x in [3, 4] else 4 if x in [5, 6] else x for x in user_type]
        user_type_indices = torch.tensor(user_type, dtype=torch.long).cuda(device_controller.get_device()) - 1       # 原本的范围是1～6，-1之后是0～5。
        user_type_onehot = F.one_hot(user_type_indices, num_classes=4)
    ...
    def get_probs(self, input):
        ...
        user_type = cur_features['user_type']  # 是一个list，长度是batch_size
        user_type = [3 if x in [3, 4] else 4 if x in [5, 6] else x for x in user_type]
        user_type_indices = torch.tensor(user_type, dtype=torch.long).cuda(
            device_controller.get_device()) - 1  # 原本的范围是1～6，-1之后是0～5。
        user_type_onehot = F.one_hot(user_type_indices, num_classes=4)
        cur_action_logits = self.actor_network(cur_common_input_res.detach(), user_type_onehot)
        cur_action_probs = F.softmax(cur_action_logits, dim=1)
